...
- **`public_repo` 스코프를 가진 classic PAT 하나만 안내한다.** GitHub은 fine-grained 토큰에 대해 소유하지 않은 공개 저장소를 항상 읽기 전용으로 취급하므로, 권한을 어떻게 설정해도 댓글·리뷰·Viewed가 거부된다. 이 앱의 주 사용 사례가 남의 오픈소스 번역 PR 검토이므로, fine-grained PAT는 안내에서 제외하고 왜 동작하지 않는지만 한 줄로 설명한다.
- 비공개 저장소까지 검토한다면 `repo` 스코프가 필요하다.
- 토큰 생성 링크와 짧은 만료 기간을 안내한다.
- fine-grained 토큰도 입력 자체는 막지 않는다. 접두어로 기능을 판단하지 않는다는 원칙(아래)과 충돌하고, 본인 소유 저장소에서는 Pull requests: Read and write가 있으면 동작하기 때문이다. 거부되면 오류 메시지가 classic PAT를 안내한다.
- 토큰 접두어만 보고 기능을 판단하지 않고 실제 API 요청으로 권한을 검증한다.
- `Test connection` 버튼을 제공한다.
- 검증에 성공하면 사용자 로그인명과 아바타, 저장소 접근 여부를 표시한다. **쓰기 가능 여부는 표시하지 않는다.** fine-grained 토큰에 부여된 권한은 사전에 조회할 수 없고, 읽기 성공을 근거로 쓰기를 약속하면 §10이 금지하는 "권한 오류를 성공으로 오인하게 표시"에 해당한다.
...
- 문서에서 짧은 만료 기간을 권장하고, classic PAT가 계정 전체 공개 저장소에 쓰기 권한을 준다는 위험을 설명한다.
