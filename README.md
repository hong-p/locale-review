...
**개발 중입니다.** 토큰 설정, PR 조회, 번역 파일 감지, locale 필터, 3열 diff 비교까지 화면에서 동작합니다.

Viewed·기존 댓글·리뷰 제출·초안 보호는 **로직과 테스트가 완료됐고 화면 연결이 남아 있습니다.** 배포 파이프라인은 준비됐지만 remote가 없어 아직 한 번도 실행되지 않았습니다. 전체 범위와 진행 순서는 [plan.md](./plan.md)에 정의되어 있습니다.
